...
*/
    ...
    function makePermaiframe() {
	    var url2 = window.location.href.replace('index-s.html', '')
		var url2 = url2.replace('#', ''),
            indexHash2 = url2.indexOf('?');
        if(indexHash2 !== -1) {
            url2 = url2.substring(0, indexHash2);
        }
        return url2 + 'iframe-s.html?' + $scope.years.indexOf($scope.selection.year) + '.' + $scope.dates.indexOf($scope.selection.date) + '.' + $scope.files.indexOf($scope.selection.file)
    }
